...
void AP_HeadWindLanding::calc_index_landing_waypoint(void)
{
    ...
    for(int16_t i=1; i<num_cmd; i++)
    {
		...
		if(current_cmd.id == MAV_CMD_NAV_LAND)
		{
		    ...
		    if(is_terrain_alt_mode)
		    	gcs().send_text(MAV_SEVERITY_NOTICE,"HWP Terrain Landing");
			break;
		}
    }
...
void AP_HeadWindLanding::generate_hw_waypoints(const MC& cmd)
{
    ...
    if(cmd.index >= idx_hwp && all_conditions_satisfied())
    {
		...
		loc_hwp1.options = land_wp.options; // same options as land wp
		...
		loc_hwp2.options = land_wp.options; // same options as land wp
		//1<<0;
		...
		loc_hwp3.options = land_wp.options; // same options as land wp
		//
		...
		if(use_hwp4)
		{
			...
			loc_hwp4.options = land_wp.options; // same options as land wp
			//
			hwp4 = last_mwp;
